...
public interface BeanFactory {
    ...
    /**
     * <p>返回指定bean的一个实例，该实例可能是共享的或独立的。
     * <p>行为与getBean(String)相同，但通过抛出BeanNotOfRequiredTypeException来提供一定程度的类型安全（如果bean不是所需的类型）。
     * 这意味着，在正确转换结果时不会抛出ClassCastException，而使用getBean(String)时可能会发生这种情况。
     *
     * <p>将别名解析为对应的规范bean名称。
     * <p>如果在本工厂实例中找不到该bean，将会向父工厂查询。
     * @param name 要获取的bean的名称
     * @param requiredType bean必须匹配的类型；可以是接口或超类
     * @return bean的实例。注意返回值永远不会为null。
     * 如果所请求的bean解析自工厂方法且返回了null存根，
     * 则会针对NullBean存根抛出BeanNotOfRequiredTypeException。
     * 对于可选依赖项的解析，建议考虑使用getBeanProvider(Class)方法。
